...
// weight layout [C_out, C_in, K], output [B, C_out, L_out]. Same math as
// workers/kernels/conv.ts; scalar-only for now, SIMD pass later.
        ...
        for co in 0..c_out {
            ...
            for lo in 0..l_out {
                ...
                for ci in 0..c_in {
                    ...
                    let w_ci = w_co.add(ci * w_stride_ci);
                    for kk in 0..k {
...
// Weight layout [C_in, C_out, K]. Init output with bias (or zero), then scatter.
// Caller must ensure output regions for [start_batch, end_batch) do not alias
// other workers' regions — we split by batch just like Conv1d.
#[no_mangle]
    ...
    for b in start_batch..end_batch {
        ...
        for ci in 0..c_in {
            ...
            for li in 0..l_in {
                ...
                for co in 0..c_out {
                    let out_base = out_b.add(co * out_stride_c);
                    let w_base = w_ci.add(co * w_stride_co);
                    for kk in 0..k {
