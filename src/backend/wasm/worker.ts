...
function executeKernel(
  ...
  if (op === "LSTM_STEP") {
    ...
    if (inputs.length > 7) {
      const cOut = inputs[7];
      hOutPtr = output.offset + (params.hNewOffElements ?? 0) * 4;
      cOutPtr = cOut.offset + (params.cNewOffElements ?? 0) * 4;
    } else {
      hOutPtr = output.offset;
      cOutPtr = output.offset + batchSize * hidden * 4;
    }
    exports.lstm_step(
