...
- **The code is the source of truth.** If your code needs a paragraph to explain _what_ it does, rewrite the code.
...
- **No Awaits in Loops:** Never `await` inside a `map` or `forEach` loop unless you specifically need sequential execution (and if you do, leave a comment explaining _why_). Use `Promise.all` for concurrent execution.
...
- **Zero-Copy is King:** When passing large datasets to workers, _always_ use `SharedArrayBuffer` or transfer ownership (`Transferable` objects) to avoid structured cloning overhead.
...
- **Read the Errors:** Do not simply ignore red squiggles. If the linter or TypeScript compiler yells at you, take the time to read the actual error message and understand _why_ it is complaining.
