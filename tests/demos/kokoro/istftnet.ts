...
export class Decoder extends Module {
  ...
  async forward(
    ...
    for (const block of this.decode) {
      await stage(`dec.block[${blockIdx}]`);
      let blockIn = x;
      if (addRes) {
